...
export default function MeditationTimer({ user, token, onRequireLogin }: Props) {
  ...
  useEffect(() => {
    ...
    if (user && token) {
      fetch('/api/logs', {method:'POST', headers:{'content-type':'application/json', authorization:`Bearer ${token}`}, body: JSON.stringify({date: todayKey(), type:'meditation', value: minutes})}).catch(e=>console.error(e))
    } else {
      onRequireLogin?.()
    }
